...
varImpPlot(forest) #or forest$importance[order(-forest$importance), , drop = FALSE]
#forest$importance conatains the same of varImpPlot(forest) but unordered


#Another metric that we can look at is the number of times, aggregated over all of the trees in the random forest model, that a certain variable is selected for a split
